...
// This structure is used when filtering past events
// and filering real-time events as well.
type EventFilter struct {
	// Match events that happened after this time. This is
	// required for filtering both past and real-time events.
	Start float64 `json:"start,omitempty"`

	// Match events that happened before this timestamp. Optional.
	End *float64 `json:"end,omitempty"`

	// Tags is a list of possible values to match for tags. Each
	// value may be a regular expression. Matches the event only if
	// all patterns are found in the event tag list.
	Tags []string `json:"tags,omitempty"`

	// Match the content of the event. This value is evaluated as
	// regular expression.
	Content *string `json:"content,omitempty"`

	// Order in which to return the events. Makes sense only for
	// past events.
	Order *string `json:"order,omitempty"`
}

// DumpBytes serializes the event filter values as bytes.
// Theia expects the filter in JSON format, so this function
// serializes the filter data to JSON, then encodes in
// UTF-8.
...
type EventResponse struct {
	...
	// The error that ocurred. If the read was successful, this will be set to nil.
	Error error
}

// Client interface describes the client API for a theia server.
// Defines methods to send events and to query both past and real-time
// events.
// The querying operations (Receive, Find) are both streaming and asynchronous,
// meaning that the number of events to be returned is not known and the
// server returns (streams) the events as the arrive. These functions
// return a chan to listen on, and events are decoded and published on the
...
type Client interface {
	...
	// error is returned.
	// It should be noted that the server will never close this type
	// of channel. The responsibility for closing the connection is on the client side.
